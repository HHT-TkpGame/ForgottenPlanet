...
public class GenerateScriptableObjectMenu : EditorWindow
{
	...
	private void OnGUI()
	{
		...
		outPutCsvFilePath = Application.dataPath + "/Scripts/SpreadSheet/Data/" + scriptableObjectName + ".csv";
		outPutCSFilePath = Application.dataPath + "/Scripts/SpreadSheet/Data/" + scriptableObjectName + ".cs";
	...
	void GenerateScriptableObjectCS(string[,] ParseCsvData)
	{
		using (var sw = new StreamWriter(outPutCSFilePath))
		{
			...
			sw.WriteLine("        string[,] data = CsvUtility.LoadCsvAs2DArray(filePath);");
